// notify-friend-accepted — data-only FCM when someone accepts a friend request.
...
Deno.serve(async (request) => {
  try {
    const { request_id, sender_id, receiver_id } = await request.json();
    ...
    if (!friendRequest || friendRequest.status !== "accepted") {
      return new Response(JSON.stringify({ skipped: "not accepted" }), {
        status: 200,
      });
    }
    ...
    let avatarUrl = "";
    if (accepter?.avatar_url) {
      const { data: signedAvatar } = await supabase.storage
        .from("avatars")
        .createSignedUrl(accepter.avatar_url, SIGNED_URL_TTL_SECONDS);
    ...
    const data: Record<string, string> = {
      ...
      avatarUrl,
      notificationTitle: accepterName,
      notificationBody: "accepted your friend request. Send them your first moment!",
    };
